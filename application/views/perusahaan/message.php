<div class="selected-user">
	<span>To: <span class="name"><?= $freelance['username'] ?></span></span>
</div>
<div class="chat-container">
	<script src="https://ajax.googleapis.com/ajax/libs/jquery/3.3.1/jquery.min.js"></script>
	<div style="height: 400px; overflow: auto; width: 100%;" id="scroll" class="live<?= $freelance['id'] ?>">
		<ul class="chat-box chatContainerScroll">
			<?php foreach ($chat as $ball): ?>
				<?php if ($ball->id_user_from == $freelance['id']): ?>
					<li class="chat-left">
						<div class="chat-avatar">
							<img src="<?= base_url('assets/img/profil/'.$freelance['image']) ?>" alt="Retail Admin">
							<div class="chat-name"><?= $freelance['username'] ?></div>
						</div>
    ...
    <div class="form-group mt-3 mb-0">
    	<form action="" method="" onsubmit="submitChat()">
            ...
            <button type="button" class="btn btn-info float-right mt-1" id="kirim-chat" onclick="kirimChat(<?= $freelance['id'] ?>)">Kirim</button>
    	</form>
    </div>
</div>
<script type="text/javascript">
	...
	$(document).ready(function() {
		setInterval(function() {
			$('#live<?= $freelance['id'] ?>').load('<?= base_url("Perusahaan/getChat2/$freelance[id]") ?>')
		}, 1000);
	});
